...
def get_high_volume_tickers(min_volume=500000, max_tickers=600):
    ...
    nasdaq = yf.Tickers(' '.join(sp500)).tickers
    tickers = sp500
    ...
    # Filter by volume
    high_volume_tickers = []
    for ticker in tickers[:max_tickers * 2]:  # Check more to ensure enough qualify
        try:
...
def main():
    ...
    if len(tickers) < 500:
        logger.error("Insufficient high-volume tickers")
        return
