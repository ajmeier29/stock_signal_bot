...
class FvgProLongAndShort(bt.Strategy):
    params = dict(
    ...
    def next(self):
        for d in self.datas:
            ...
            bearish_fvg = d.high[0] < d.low[-2]
            bullish = d.close[0] > d.open[0] and (d.close[0] - d.open[0]) > 0.5 * (d.high[0] - d.low[0])
            bearish = d.close[0] < d.open[0] and (d.open[0] - d.close[0]) > 0.5 * (d.high[0] - d.low[0])
            ...
            if not pos:
                ...
                elif bearish_fvg and bearish and price < sma:
                    self.sell(data=d, size=size)
                    self.entry_price[d] = price
                    self.low_water[d] = price
                    self.break_even_triggered[d] = False
                    self.entry_bar[d] = len(d)
                    print(f"📅 SHORT {d._name} @ {price:.2f}")

            # === POSITION MANAGEMENT ===
            elif pos:
                entry = self.entry_price[d]
                bars_held = len(d) - self.entry_bar[d]
                pnl = (price - entry) * pos.size if pos.size > 0 else (entry - price) * abs(pos.size)
                ...
                if pos.size > 0:
                    ...
                    self.low_water[d] = min(self.low_water[d], price)

                    if not self.break_even_triggered[d] and price <= entry * (1 - self.p.partial_tp_pct):
                        self.buy(data=d, size=abs(pos.size) // 2)
                        print(f"📉 TP Partial SHORT {d._name} @ {price:.2f} | PnL: {pnl:.2f}")
                        self.break_even_triggered[d] = True

                    if self.break_even_triggered[d]:
                        trail_stop = self.low_water[d] + self.p.atr_mult_trail * atr
                        if bars_held >= self.p.max_bars_in_trade:
                    ...
                    if price >= trail_stop:
                        self.close(data=d)
                        print(f"🚪 EXIT SHORT {d._name} @ {price:.2f} | PnL: {pnl:.2f}")
...
#                     if not self.break_even_triggered[d] and price >= entry * (1 + self.p.partial_tp_pct):
#                         self.sell(data=d, size=pos.size // 2)
#                         print(f"📈 TP Partial LONG {d._name} @ {price:.2f}")
#                         self.break_even_triggered[d] = True
...
#                     if price <= trail_stop:
#                         self.close(data=d)
#                         print(f"🚪 EXIT LONG {d._name} @ {price:.2f}")
...
#                     if not self.break_even_triggered[d] and price <= entry * (1 - self.p.partial_tp_pct):
#                         self.buy(data=d, size=abs(pos.size) // 2)
#                         print(f"📉 TP Partial SHORT {d._name} @ {price:.2f}")
#                         self.break_even_triggered[d] = True
...
#                     if price >= trail_stop:
#                         self.close(data=d)
#                         print(f"🚪 EXIT SHORT {d._name} @ {price:.2f}")
