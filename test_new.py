...
#     retries = 3  # Increased retries for failed tickers
...
#         # Fallback to historical data for empty batches
#         if not bars.empty:
#             found_tickers = bars['symbol'].unique().tolist()
#             missing_tickers = [t for t in batch if t not in found_tickers]
#             if missing_tickers:
#                 try:
...
#                         logger.info(f"Fetched {len(hist_bars)} historical bars for {missing_tickers}")
